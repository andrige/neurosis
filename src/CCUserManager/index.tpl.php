...
<?php foreach($users as $val):?>
  <tr><td><?=htmlent($val['name'])?></td><td><?=htmlent($val['email'])?></td><td><?=htmlent($val['acronym'])?></td><td><?=htmlent($val['created'])?></td><td><a href='<?=create_url('usermanager/user/'.htmlent($val['id']))?>'>Edit</a></td></tr>
<?php endforeach;?>
</div>
</table>